...
- **Integrated Console** — run terminal commands directly from Monaco (Ctrl+J to toggle); execute scripts, build tools, and dev workflows without leaving the editor. On Windows: choose default shell (Windows PowerShell, PowerShell Core, or CMD) and cycle shells; drag & drop files/folders from OS into console. The console display shows the active shell with clear labels (PS, PWSH, CMD).
...
- **Run commands**: Type and press Enter; `cd` changes directory, `pwd` shows current directory, other commands run there; Shift+Enter for multi-line
