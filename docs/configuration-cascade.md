...
Previous versions had a fourth level with hardcoded defaults in `monacoHtml.js` (e.g., `PRETTIER_PRINT_WIDTH = 80`). These were removed because:
...
- `monacoHtml.js` — Configuration variables (no defaults, set by `applyEditorConfig()`)
