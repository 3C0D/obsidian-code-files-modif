 ...
 */
export function buildMessageHandler(ctx: MessageHandlerContext): (event: MessageEvent) => Promise<void> {
    const { iframe, send, valueRef, codeContext, plugin, initParams, loadProjectFiles, autoFocus, onChange, onSave, onFormatDiff, onFormatDiffReverted, onOpenEditorConfig, onOpenThemePicker, onOpenRenameExtension } = ctx;
