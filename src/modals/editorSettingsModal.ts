  ...
  async onOpen(): Promise<void> {
    ...
    if (isJsTs) {
      new Setting(toggleSection)
        ...
        .addToggle((t) =>
          t.setValue(this.plugin.settings.semanticValidation).onChange(async (v) => {
    ...
    if (!isFormattableVal) {
      ...
      nonFormattableSection.createEl('hr', {
        attr: {
          style:
            'margin: 0 0 1rem 0; border: none; border-top: 1px solid var(--background-modifier-border);'
        }
      });
    ...
    formatterSection.createEl('hr', {
      attr: {
        style:
          'margin: 0 0 0.5rem 0; border: none; border-top: 1px solid var(--background-modifier-border);'
      }
    });
