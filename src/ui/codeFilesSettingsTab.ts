...
export class CodeFilesSettingsTab extends PluginSettingTab {
  ...
  private renderExtensionsSection(containerEl: HTMLElement): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.allExtensions).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Maximum file size in MB for opening files in Monaco (default: 10 MB, min: 0.1 MB)'
      )
      .addText((text) => {
        text.setPlaceholder('10').setValue(String(this.plugin.settings.maxFileSize));
        ...
        // Restore the last valid saved value if the field is left with an invalid input
        text.inputEl.addEventListener('blur', () => {
          const value = parseFloat(text.getValue());
          if (isNaN(value) || value < 0.1 || value > 100) {
  ...
  private renderEditorConfigSection(containerEl: HTMLElement): void {
    ...
    scopeRow.createEl('span', {
      ...
      attr: {
        style:
          'margin-left: auto; margin-right: 8px; color: var(--text-muted); font-size: 0.9em;'
      }
    });
    ...
    const editorContainer = containerEl.createEl('div', {
      attr: {
        style:
          'border: 1px solid var(--background-modifier-border); border-radius: 4px; overflow: hidden; height: 190px; margin-top: 8px;'
      }
    });
